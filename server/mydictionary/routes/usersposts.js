...
router.post('/getUsersPosts', function (req, res) {
    ...
    const result5 = db.prepare(`SELECT usersposts.id, usersposts.type, usersposts.date, usersposts.userId, tasksforfriends.senderId, tasksforfriends.receiverId, tasksforfriends.taskLanguage, tasksforfriends.wordCounter, tasksforfriends.trueAnswerCounter, users1.name AS senderName, users2.name AS receiverName FROM usersposts JOIN tasksforfriends ON usersposts.userId='${req['body']['userId']}' and usersposts.type='TaskForFriend' and usersposts.postId=tasksforfriends.id JOIN users users1 ON tasksforfriends.senderId=users1.id JOIN users users2 ON tasksforfriends.receiverId=users2.id`).all()
    const result6 = db.prepare(`SELECT usersposts.id, usersposts.type, usersposts.date, usersposts.userId, tasksforfriends.senderId, tasksforfriends.receiverId, tasksforfriends.taskLanguage, tasksforfriends.wordCounter, tasksforfriends.trueAnswerCounter, users1.name AS senderName, users2.name AS receiverName FROM subscribers JOIN users users3 ON subscribers.subscription=users3.id JOIN usersposts ON subscribers.subscriber='${req['body']['userId']}' and usersposts.userId=subscribers.subscription JOIN tasksforfriends ON usersposts.userId=subscribers.subscription and usersposts.type='TaskForFriend' and usersposts.postId=tasksforfriends.id JOIN users users1 ON tasksforfriends.senderId=users1.id JOIN users users2 ON tasksforfriends.receiverId=users2.id`).all()
