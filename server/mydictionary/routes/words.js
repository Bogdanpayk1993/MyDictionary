...
router.post('/getId', function (req, res) {
    const result = db.prepare(`SELECT * FROM words WHERE id='${req['body']['id']}'`).all()
    res.send(result)
})

router.post('/getEnglishUkrainian', function (req, res) {
    const result = db.prepare(`SELECT * FROM words WHERE english='${req['body']['english']}' and ukrainian='${req['body']['ukrainian']}'`).all()
    res.send(result)
})
